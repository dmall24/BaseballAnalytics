...
```{r}
battrackingeos <- read.csv("C:/Users/eazyd/OneDrive/Desktop/writing stuff/baseballwriting/bat-tracking_eos.csv",header = TRUE)
basicstatseos <- read.csv("C:/Users/eazyd/OneDrive/Desktop/writing stuff/baseballwriting/stats_eos.csv",header = TRUE)
exitveloeos <- read.csv("C:/Users/eazyd/OneDrive/Desktop/writing stuff/baseballwriting/exit_velocity_eos.csv",header = TRUE)
xstatseos <- read.csv("C:/Users/eazyd/OneDrive/Desktop/writing stuff/baseballwriting/expected_stats_eos.csv",header = TRUE)

IDCHECK <- data.frame(
ID = c(basicstatseos$player_id,
battrackingeos$ï..id,
exitveloeos$player_id,
xstatseos$player_id)
)
numIDs <- table(IDCHECK$ID)

eosstatsfilt <- basicstatseos %>% filter(player_id %in% usableids)
eostrackingfilt <- battrackingeos %>% filter(ï..id %in% usableids)
eosvelofilt <- exitveloeos %>% filter(player_id %in% usableids)
eosxstatsfilt <- xstatseos %>% filter(player_id %in% usableids)
...
eosstatfinal <- eosstatsfilt %>%
dplyr::select("ï..last_name..first_name", "ab", "pa", "single", "double", "triple", "home_run", "strikeout", "walk", "batting_avg", "slg_percent", "on_base_percent", "isolated_power", "babip")

eostrackingfinal <- eostrackingfilt %>%
dplyr::select("name", "swings_competitive", "contact", "avg_bat_speed", "hard_swing_rate", "squared_up_per_swing","squared_up_per_bat_contact", "blast_per_swing", "blast_per_bat_contact", "swing_length", "swords", "whiffs","whiff_per_swing")

eosvelofinal <- eosvelofilt %>%
dplyr::select("ï..last_name..first_name","avg_hit_angle", "avg_hit_speed","avg_distance","max_hit_speed","max_distance", "avg_hr_distance", "barrels","brl_percent","brl_pa")

eosxstatsfinal <- eosxstatsfilt %>%
dplyr::select("ï..last_name..first_name","bip","est_ba","est_slg", "woba","est_woba")

namechange <- c("name", "ab", "pa", "1b", "2b", "3b","hr","k","bb","ba","slg","obp","iso","babip")

names(basicsfinal) <- namechange
names(eosstatfinal) <- namechange
...
names(eostrackingfinal) <- namechange2

names(exitvelofinal) <- namechange3
names(eosvelofinal) <- namechange3

names(xstatsfinal) <- namechange4
names(eosxstatsfinal) <- namechange4
...
c <- eosstatfinal %>%
inner_join(eosxstatsfinal, by = c("name"))
d <- c %>%
inner_join(eostrackingfinal, by = c("name"))
eosbattingfinal <- d %>%
inner_join(eosvelofinal, by = c("name"))
